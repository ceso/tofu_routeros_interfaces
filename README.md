...
unit "interfaces" {
  ...
  values = merge(
    ...
    {
      configure_vlans              = local.router_defaults.create_vlans
      configure_wan_internet       = local.router_defaults.set_wan
      create_vlan_bridge_interface = local.router_defaults.create_vlan_bridge_interface
    },
    # Device-specific vlans/ethernet interfaces values
    {
      vlans               = local.interfaces.vlans
      ethernet_interfaces = local.interfaces.ethernet_interfaces
    }
  )
}
```
...
| <a name="input_wan_pppoe_credentials"></a> [wan\_pppoe\_credentials](#input\_wan\_pppoe\_credentials) | PPPoE client credentials | <pre>object({<br/>    username = optional(string)<br/>    password = optional(string)<br/>  })</pre> | n/a | yes |
